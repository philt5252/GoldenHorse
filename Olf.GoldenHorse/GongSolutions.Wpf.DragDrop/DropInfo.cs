        ...
        if (item != null) {
          ...
          if (this.VisualTargetOrientation == Orientation.Vertical) {
            ...
            if (currentYPos > targetHeight * 0.25 && currentYPos < targetHeight * 0.75) {
                ...
                ITestItemViewModel testItemViewModel = (item as Control).DataContext as ITestItemViewModel;

                if (testItemViewModel != null && testItemViewModel.Operation.Contains("Data"))
                {
                    
                }

                if (testItemViewModel != null && testItemViewModel.SupportsChildren)
                {
                    if (item is ItemsControl)
                    {
                        this.InsertPosition |= RelativeInsertPosition.TargetItemCenter;
                        ItemsControl childItemControl = item as ItemsControl;
                        this.InsertIndex = 0;//itemParent.ItemContainerGenerator.IndexFromContainer(item);
                        this.TargetCollection = childItemControl.ItemsSource ?? childItemControl.Items;
                        this.TargetItem = null;//itemParent.ItemContainerGenerator.ItemFromContainer(item);
                        this.VisualTargetItem = item;
                    }
                }
            }
